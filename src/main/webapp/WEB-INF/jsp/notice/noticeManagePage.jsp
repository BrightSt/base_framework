	...
	<div class="div-a">
		...
		<form class="form-inline" role="form">
			<div class="form-group">
				&nbsp;&nbsp;&nbsp;&nbsp;通知公告主题：
		      <input type="text" class="form-control" id="searchName" name="searchName" style="width:250px;margin-right: 20px;"maxlength="50">
	        </div>
	        <div class="form-group" style="margin-left: 10px;">
	        	<button type="button" class="btn btn-success" onclick="searchBtn();" style="margin-right: 10px;"><span class="glyphicon glyphicon-search"></span>查询</button>
				<button type="button" class="btn btn-primary" onclick="addInfo();" style="margin-right: 10px;"><span class="glyphicon glyphicon-plus"></span>增加</button>
				<button type="button" class="btn btn-warning" onclick="updateInfo();" style="margin-right: 10px;"><span class="glyphicon glyphicon-edit"></span>修改</button>
           	 	<button type="button" class="btn btn-info" onclick="delInfo();" style="margin-right: 10px;"><span class="glyphicon glyphicon-remove"></span>删除</button>
			</div>
		</form>
	</div>
	<div class="row-fluid">
          	...
          	<input type="hidden" name="id" id="updateId"/>
       		<aside class="side-right" id='third_side_right'>
					...
					<div class="form-group" style="margin-left: 10px;">
						...
						<input type="text" class="form-control" style="width:750px;" id="updateName" placeholder='主题...' maxlength="250">
					</div>
					<!--- 输入框 --->
					<div class="form-group" style="margin-left: 10px;">
				...
				<div class="form-group" style="margin-left: 10px;  ;">
					...
					<select type="text" class="form-control" id="createName" name="createName" style="width:25%;">
...
$(function(){
	...
	shi.selectpicker({
  	...
  	function searchBtn(){
  		...
  		$('#dt').bootstrapTable({
			...
			url:'<%=basePath%>notice/findAllRegs.do',
		    ...
		    },
		    {
                ...
                valign:'middle',
                formatter:function(value,data,index){
                	var str = '<button style="margin-top: 5px;" onclick="searchRegulations(\''+data.id+'\');" type="button"  class="btn btn-info btn-xs" style="margin-right:10px;">查看详情</button>';
                	return str;
            	}
		    }  
		    ],
 ...
 function addInfo(){
	 ...
	 window.location.href="<%=basePath%>notice/toAdd.do"
 }
//删除
function delInfo(){
	var ids = getSelectionsStr();
		if(ids==""){
			...
			bootbox.confirm("您确定要删除该数据吗？",function(result){
   			if(result){
   				$.commonReq({
	  				url : rootPath + "/notice/deleteRegs.do",
	  				async : true,
	  				data : {"ids":ids},
 ...
 //修改通知公告
 	function updateInfo(){
       ...
       // $("#uploadFileId").val(id);
        $.commonReq({
				url : rootPath + "/notice/selectById.do",
				async : true,
				data : {"id":id},
				success : function(parame) {
     ...
     /*  //检查关键字
      ...
      //附件
  		...
  		if(flag){
  			 $.commonReq({
 				url : rootPath + "/notice/updateNoticeInfo.do",
 				async : true,
 				data:{
                	"id":id,
                	"body":body,
 				...
 				success : function(data) {
                        ...
                        $('#dt').bootstrapTable('refresh', {url:rootPath + '/notice/findAllRegs.do'});
                        bootbox.alert("修改成功！");
                },
